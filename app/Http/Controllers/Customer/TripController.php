...
class TripController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        // Lấy query cơ bản
        $tripsQuery = Trip::with(['route.departureLocation', 'route.destinationLocation', 'vehicle'])
        ...
        // Lọc theo ngày nếu có, nếu không thì lấy các chuyến từ hôm nay trở đi
        if ($request->filled('trip_date')) {
            $tripsQuery->where('trip_date', $request->trip_date);
        } else {
            $tripsQuery->where('trip_date', '>=', now()->toDateString());
        }

        // Sắp xếp ngày gần nhất và giờ sớm nhất
        $trips = $tripsQuery->orderBy('trip_date', 'asc')->orderBy('departure_time', 'asc')->get();

...
class ParcelController extends Controller
{
    ...
    public function create()
    {
        ...
        $routes = Route::where('status', 'active')->orWhere('status', '1')->get(); 
        // Fallback to all routes if status does not exist
        if ($routes->isEmpty()) {
            $routes = Route::all();
        }
    ...
    public function store(Request $request)
    {
        ...
        // Calculate price based on weight (e.g. 10000 per kg)
        // Adjust this logic based on your actual pricing rules
        $pricePerKg = 10000;
        $data['price'] = max(20000, $data['weight'] * $pricePerKg); // Minimum 20k

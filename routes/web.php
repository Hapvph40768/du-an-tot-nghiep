...
Route::prefix('driver')->name('driver.')->middleware(['auth', CheckDriverRole::class])->group(function () {
    ...
    // Sidebar features: Lịch sử và Doanh thu
    Route::get('/history', [DriverTripController::class, 'history'])->name('history.index');
    ...
    Route::post('/trips/{trip}/checkin', [DriverTripController::class, 'checkinTicket'])->name('trips.checkin');
    
    // Thu tiền mặt 
    Route::post('/tickets/{ticket}/pay', [DriverTripController::class, 'collectCash'])->name('tickets.pay');
    // Mua vé dọc đường (Walk-in)
    Route::post('/trips/{trip}/walk-in', [DriverTripController::class, 'storeWalkIn'])->name('trips.walkin');
...
|--------------------------------------------------------------------------
*/
use App\Http\Controllers\Assistant\AssistantTripController;
use App\Http\Controllers\Assistant\HomeController as AssistantHomeController;
use App\Http\Middleware\CheckAssistantRole;

Route::prefix('assistant')->name('assistant.')->middleware(['auth', CheckAssistantRole::class])->group(function () {
    Route::get('/profile', [AssistantHomeController::class, 'profile'])->name('profile');

    // Sidebar features: Lịch sử và Doanh thu
    Route::get('/history', [AssistantTripController::class, 'history'])->name('history.index');
    Route::get('/revenue', [AssistantTripController::class, 'revenue'])->name('revenue.index');

    Route::get('/trips', [AssistantTripController::class, 'index'])->name('trips.index');
    Route::get('/trips/{trip}', [AssistantTripController::class, 'show'])->name('trips.show');
    Route::post('/trips/{trip}/checkin', [AssistantTripController::class, 'checkinTicket'])->name('trips.checkin');
    
    // Thu tiền mặt 
    Route::post('/tickets/{ticket}/pay', [AssistantTripController::class, 'collectCash'])->name('tickets.pay');
    // Mua vé dọc đường (Walk-in)
    Route::post('/trips/{trip}/walk-in', [AssistantTripController::class, 'storeWalkIn'])->name('trips.walkin');

    Route::get('/trips/{trip}/start', [AssistantTripController::class, 'start'])->name('trips.start');
});

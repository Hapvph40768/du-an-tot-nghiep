...
@section('content-main')
    <div class="max-w-7xl mx-auto px-4 py-10">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
            <div class="flex items-center gap-4">
                <a href="{{ route('assistant.trips.index') }}"
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="bg-white rounded-3xl shadow border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-8">
                        <div class="flex justify-between items-start">
                            <div>
                                <span
                                    class="inline-flex items-center gap-3 px-6 py-3 bg-blue-50 text-blue-700 rounded-3xl text-xl font-semibold">
                        ...
                        <div class="mt-12 flex items-center gap-8">
                            ...
                            <div class="flex flex-col items-center text-blue-500">
                                ...
                                <div class="w-24 h-px bg-gradient-to-r from-transparent via-blue-400 to-transparent my-2">
                                </div>
                            </div>
                ...
                <div class="bg-white rounded-3xl shadow border border-gray-100 p-8">
                    <h2 class="flex items-center gap-3 text-xl font-semibold mb-6">
                        <i class='bx bx-bus text-2xl text-blue-500'></i>
                ...
                <div class="grid lg:grid-cols-2 gap-8 mb-8">
                    ...
                    <div class="bg-white rounded-3xl shadow border border-gray-100 p-6 sm:p-8">
                        <h2 class="text-xl font-semibold mb-6 flex items-center gap-2">
                            <i class='bx bx-category text-blue-500'></i> Sơ đồ ghế
                        </h2>
                        ...
                        <div class="grid grid-cols-4 gap-3 max-w-xs mx-auto relative p-5 bg-gray-50 rounded-3xl border-2 border-gray-200" style="border-top-width: 8px; border-top-color: #cbd5e1;">
                            ...
                            @php
                                ...
                                foreach($trip->tickets->where('status', '!=', 'cancelled') as $t) {
                                    ...
                                    $ticketDataJSON[$sNum] = [
                                        'ticket_id' => $t->id,
                                        'name' => $t->passenger_name ?? ($t->booking?->contact_name ?? ($u->name ?? 'Khách vãng lai')),
                                        'phone' => $t->booking?->contact_phone ?? ($u->phone ?? '--'),
                                        'code' => $t->ticket_code,
                                        'status' => $t->status,
                            ...
                            @foreach($vehicleSeats as $seat)
                                @php
                                    ...
                                    $bgClass = $isUsed ? 'bg-emerald-500 text-white border-emerald-600 shadow-sm' :
                                              ($isBooked ? 'bg-blue-400 text-white border-blue-500 shadow-sm' : 'bg-white text-gray-500 border-gray-200 hover:border-blue-300');
                                @endphp
                                <div class="aspect-square rounded-xl border-2 flex items-center justify-center font-bold text-base {{ $bgClass }} cursor-pointer transition-all hover:scale-105" 
                                     onclick="handleSeatClick('{{ $seat->seat_number }}', '{{ $seat->id }}', {{ $isBooked ? 'true' : 'false' }})"
                        ...
                        <div class="mt-8 flex justify-center gap-4 text-xs font-medium text-gray-600">
                            ...
                            <div class="flex items-center gap-1.5"><div class="w-3.5 h-3.5 rounded bg-blue-400 border border-blue-500"></div> Đã đặt</div>
                        ...
                        <div id="passengerData" class="hidden flex-col h-full space-y-5">
                            <div class="flex items-center justify-between border-b border-gray-100 pb-4">
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-2xl font-bold text-blue-600 mt-0.5" id="pd-seat">--</p>
                                </div>
                            </div>
                            ...
                            <div class="bg-blue-50/50 rounded-2xl p-4 border border-blue-100">
                                ...
                                <p class="font-medium text-gray-900 flex items-start gap-2">
                                    <i class='bx bx-map-pin text-blue-500 mt-0.5'></i>
                            ...
                            <div class="mt-auto pt-4 flex items-center justify-between border-t border-gray-100">
                                ...
                                <div>
                                    <button id="btn-collect-cash" onclick="collectCash()" class="hidden px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-xl text-sm transition-all shadow-sm">
                                    ...
                                    @foreach ($trip->tickets->where('status', '!=', 'cancelled') as $ticket)
                                        ...
                                        <tr class="hover:bg-gray-50 transition">
                                            <td class="py-5 font-medium">
                                                {{ $ticket->passenger_name ?? ($ticket->booking?->contact_name ?? ($user?->name ?? 'Khách vãng lai')) }}
                                            </td>
                                            <td class="py-5 font-semibold text-blue-600">
                                                {{ $ticket->seat?->seat_number ?? ($ticket->seat_number ?? '—') }}
                                            </td>
                                            <td class="py-5 text-gray-600">
                                                {{ $ticket->booking?->contact_phone ?? ($user?->phone ?? '—') }}
                                            </td>
                                            ...
                                            <td class="py-5 text-center">
                                                <span
                                                    ...
                                                    {{ $ticket->status === 'confirmed' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700' }}">
            ...
            <div class="lg:col-span-4 space-y-8">
                ...
                <div class="bg-white rounded-3xl shadow border border-gray-100 p-8 sticky top-8">
                    ...
                    @if ($trip->status === 'active')
                        <div
                            class="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 rounded-3xl p-7 text-center mb-8">
                            <p class="text-blue-700 font-medium">Thời gian còn lại</p>
                            <p id="countdown" class="text-5xl font-bold text-blue-600 mt-3 tracking-tighter">—</p>
                            <p class="text-sm text-blue-600 mt-1">trước khi khởi hành</p>
                        </div>
                    @endif
                    ...
                    <div class="space-y-4">
                        @if ($trip->status === 'active')
                            <button
                                ...
                                window.location.href = '{{ route('assistant.trips.start', $trip) }}'"
                        ...
                        <button onclick="showRouteModal()"
                            class="w-full py-4 border border-blue-500 text-blue-600 font-semibold rounded-2xl hover:bg-blue-50 transition flex items-center justify-center gap-2">
                    ...
                    @forelse ($trip->pickupPoints->sortBy('pivot.order') as $index => $point)
                        <div class="relative flex gap-6 group">
                            <div class="absolute -left-12 top-3 flex flex-col items-center">
                                <div
                                    class="w-7 h-7 rounded-full bg-blue-500 flex items-center justify-center text-white text-sm font-bold shadow-md">
                                    {{ $index + 1 }}
                                </div>
                                @if (!$loop->last)
                                    <div
                                        class="w-px h-[calc(100%+48px)] bg-gradient-to-b from-blue-300 to-transparent mt-3">
                                    </div>
                                @endif
                            </div>

                            <div
                                class="flex-1 bg-white border border-gray-100 rounded-3xl p-6 hover:border-blue-300 hover:shadow-md transition-all">
                                <div class="flex justify-between items-start">
                                    <div class="flex-1">
    ...
    <div id="walkinModal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
        <div class="bg-white rounded-3xl max-w-sm w-full shadow-2xl overflow-hidden relative">
            <div class="px-6 py-5 border-b bg-gradient-to-r from-blue-50 to-white flex justify-between items-center">
                <h3 class="text-xl font-semibold text-gray-900 flex items-center gap-2">
                    <i class='bx bx-user-plus text-blue-500'></i> Khách lẻ dọc đường
                </h3>
                <button onclick="closeWalkinModal()" class="text-2xl text-gray-400 hover:text-gray-600">&times;</button>
            </div>
            <div class="p-6">
                <p class="text-sm text-gray-500 mb-4">Ghế muốn đặt: <span id="walkin-seat-badge" class="font-bold text-blue-600 text-lg ml-1"></span></p>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="walkin-name" class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="Nhập tên khách">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Số điện thoại</label>
                        <input type="text" id="walkin-phone" class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="Nếu có">
                    </div>
                    ...
                    <button onclick="submitWalkin()" id="btn-submit-walkin" class="w-full py-3 bg-blue-500 text-white font-bold rounded-xl hover:bg-blue-600 transition shadow-sm mt-2 flex justify-center items-center gap-2">
    ...
    <script>
        const ticketDataMap = {!! json_encode($ticketDataJSON ?? []) !!};
        ...
        function collectCash() {
            ...
            fetch(`/assistant/tickets/${currentSelectedTicketId}/pay`, {
                method: 'POST',
                headers: {
        ...
        function submitWalkin() {
            ...
            fetch(`/assistant/trips/{{ $trip->id }}/walk-in`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        function processCheckin(ticketCode) {
            ...
            fetch(`{{ route('assistant.trips.checkin', $trip->id) }}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}' // make sure meta csrf is present or layout injects it
                },

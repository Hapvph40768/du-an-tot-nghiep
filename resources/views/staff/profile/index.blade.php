...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
    ...
    <div class="lg:col-span-1 space-y-6">
        <div class="bg-white dark:bg-[#111111] p-8 rounded-3xl shadow-sm border border-[#e3e3e0] dark:border-[#262626] text-center">
            <div class="relative w-32 h-32 mx-auto mb-4 group">
                <img src="{{ $user->avatar ?? 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&background=3b82f6&color=fff&size=128' }}" 
            ...
            <div class="grid grid-cols-2 gap-4 pt-6 border-t border-[#e3e3e0] dark:border-[#262626] border-dashed">
                <div class="text-center">
                    <div class="text-lg font-black text-blue-600">{{ $stats['check_ins'] }}</div>
                    <div class="text-[10px] opacity-40 font-bold uppercase">Check-in</div>
                </div>
                <div class="text-center">
                    <div class="text-lg font-black text-green-600">{{ $stats['money_confirmed'] }}</div>
        ...
        <div class="bg-blue-600 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden">
            <div class="relative z-10">
                ...
                <div class="text-lg font-black">{{ $user->contract_type ?? 'Chính thức' }}</div>
            </div>
    ...
    <div class="lg:col-span-2 space-y-8">
            ...
            <form action="{{ route('staff.profile.update') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all">
                    </div>
                    <div>
        ...
        <div class="bg-gray-50 dark:bg-[#0a0a0a] p-8 rounded-3xl shadow-sm border border-[#e3e3e0] dark:border-[#262626]">
            ...
            <form action="{{ route('staff.profile.password') }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
    ...
    <div class="relative border-l-2 border-gray-100 dark:border-gray-800 ml-3 space-y-8">
        @foreach($recentLogs as $log)
            <div class="relative pl-8">
                ...
                <div class="text-[10px] font-bold opacity-40 uppercase mb-1">{{ $log->created_at->diffForHumans() }} ({{ $log->ip_address }})</div>
                <div class="text-sm font-bold">{{ $log->description }}</div>
                <div class="text-[10px] italic opacity-40">Mã hành động: {{ strtoupper($log->action) }}</div>
            </div>
        @endforeach
    </div>
</div>

            ...
            <div class="card shadow-sm border-0 rounded-4 p-4">
                ...
                <form action="{{ route('admin.vehicles.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="license_plate" class="form-control rounded-3 @error('license_plate') is-invalid @enderror" 
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="type" class="form-control rounded-3 @error('type') is-invalid @enderror" 
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" name="total_seats" class="form-control rounded-3 @error('total_seats') is-invalid @enderror" 
                                   value="{{ old('total_seats') }}" min="1" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-bold small">Số điện thoại</label>
                            <input type="text" name="phone_vehicles" class="form-control rounded-3 @error('phone_vehicles') is-invalid @enderror"
                                    ...
                                    @if($parking->slots->count() > 0)
                                        <optgroup label="{{ $parking->name }} ({{ $parking->location }})">
                                            @foreach($parking->slots as $slot)
                                                <option value="{{ $slot->id }}" {{ old('parking_slot_id') == $slot->id ? 'selected' : '' }}>
                    ...
                    <div class="mt-4 pt-3 border-top">
                        ...
                        <a href="{{ route('admin.vehicles.index') }}" class="btn btn-light px-4 border ms-2" style="border-radius: 10px; height: 45px;">Hủy</a>
                    </div>
                </form>
            </div>
        </div>

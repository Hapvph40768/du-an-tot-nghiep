...
<style>
    :root { --primary-color: #ff6b00; --primary-hover: #e65100; }} .card-box { background: #ffffff; border-radius: 16px; padding: 24px; box-shadow: 0 5px 20px rgba(0, 0, 0, 0.03); border: 1px solid #f0f0f0; }} .form-label { font-weight: 600; color: #374151; font-size: 14px; }} .btn-primary-custom { background-color: var(--primary-color); border: none; color: white; padding: 10px 25px; border-radius: 10px; font-weight: 600; transition: 0.3s; }} .btn-primary-custom:hover { background-color: var(--primary-hover); transform: translateY(-2px); color: white; }} .input-group-text { border-radius: 0 10px 10px 0; border-left: none; }} .form-control-custom { border-radius: 10px !important; }}</style>
            ...
            <div class="card-box shadow-sm">
                ...
                <form action="{{ route('admin.routes.update', $route->id) }}" method="POST">
                    ...
                    <div class="mt-5 pt-3 border-top d-flex gap-2">
                        ...
                        <a href="{{ route('admin.routes.index') }}" class="btn btn-light px-4 border" style="border-radius: 10px;">{{{ __('cancel') }} bỏ</a>
                    </div>
                </form>
            </div>
        </div>

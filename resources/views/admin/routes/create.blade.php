            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px;">
                <div>
                    ...
                    <select name="start_location_id"
                        ...
                        @forelse($locations as $location)
                            <option value="{{ $location->id }}" @selected(old('start_location_id') == $location->id)>{{ $location->name }}
                            </option>
                        @empty
                ...
                <div>
                    ...
                    <select name="end_location_id"
                        ...
                        <option value="">-- Chọn điểm --</option>
                        @forelse($locations as $location)
                            <option value="{{ $location->id }}" @selected(old('end_location_id') == $location->id)>{{ $location->name }}
                            </option>
                        @empty
                            <option value="" disabled>Chưa có địa điểm</option>
                        @endforelse
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                <div>
                    ...
                    <input type="number" name="distance_km"
                        style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px;"
                        value="{{ old('distance_km') }}" min="1" required>
                </div>
                <div>
                    ...
                    <input type="number" name="estimated_time"
                        style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px;"
                        value="{{ old('estimated_time') }}" min="1" required>
                </div>
            </div>

...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="badge bg-white border text-dark p-2 rounded-3 shadow-sm">
            <span class="text-muted">Tổng số:</span> <span class="fw-bold">{{ $reviews->total() }}</span>
        </div>
    </div>
                            ...
                            <td>
                                @if($review->booking && $review->booking->trip)
                                    <div class="trip-info mb-1">
                                        <i class='bx bx-map-pin'></i> {{ $review->booking->trip->route->departureLocation->name ?? 'N/A' }} 
                                        → {{ $review->booking->trip->route->destinationLocation->name ?? 'N/A' }}
                                    </div>
                                    <div class="small text-muted">
                                        Ngày: {{ \Carbon\Carbon::parse($review->booking->trip->trip_date)->format('d/m/Y') }}
                                    </div>
                                @else

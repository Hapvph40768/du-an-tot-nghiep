    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card shadow-sm border-0 rounded-4 p-4">
                <h4 class="fw-bold mb-4">Chỉnh sửa Khuyến mãi #{{ $promotion->id }}</h4>
                <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST">
                    @csrf @method('PUT')
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label fw-bold small">Mã khuyến mãi</label>
                            <input type="text" name="code" value="{{ old('code', $promotion->code) }}" class="form-control rounded-3 @error('code') is-invalid @enderror">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="type" class="form-select rounded-3">
                                ...
                                <option value="fixed" {{ old('type', $promotion->type)=='fixed'?'selected':'' }}>Cố định (VNĐ)</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-bold small">Giá trị giảm</label>
                            <input type="number" name="value" value="{{ old('value', $promotion->value) }}" class="form-control rounded-3 @error('value') is-invalid @enderror">

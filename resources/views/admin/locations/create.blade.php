        ...
        <form action="{{ route('admin.locations.store') }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px;">
                <div style="grid-column: span 2;">
                    ...
                    <input type="text" name="name"
                        ...
                        value="{{ old('name') }}" placeholder="Ví dụ: Bến xe Mỹ Đình" required autofocus>
                    @error('name')
                ...
                <div style="grid-column: span 2;">
                    ...
                    <input type="text" name="address"
                        style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px;"
                        value="{{ old('address') }}" placeholder="Số nhà, đường, phường/xã...">
                    @error('address')
                        <div style="color: #c33; font-size: 12px; margin-top: 4px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="city"
                        style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px;"
                        value="{{ old('city') }}" placeholder="Hà Nội, TP. Hồ Chí Minh, Đà Nẵng...">
                    @error('city')
                        <div style="color: #c33; font-size: 12px; margin-top: 4px;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="province_code"
                        style="width: 100%; padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px;"
                        value="{{ old('province_code') }}" placeholder="HN, HCM, DN...">
                    @error('province_code')
                        <div style="color: #c33; font-size: 12px; margin-top: 4px;">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div style="margin-bottom: 24px;">
                ...
                <textarea name="note" rows="4"
                    ...
                    placeholder="Ví dụ: Bến chính, có bãi đỗ xe lớn, gần trung tâm...">{{ old('note') }}</textarea>
                @error('note')
                    <div style="color: #c33; font-size: 12px; margin-top: 4px;">{{ $message }}</div>
                @enderror
            </div>

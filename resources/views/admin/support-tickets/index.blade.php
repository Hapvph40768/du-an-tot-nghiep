                    ...
                    <table class="table table-hover align-middle mb-0" id="ticketsTable">
                        ...
                        <tbody>
                            @forelse ($tickets as $ticket)
                                <tr>
                                    <td class="ps-4 fw-medium">#{{ $ticket->id }}</td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <div class="fw-semibold">{{ $ticket->user?->name ?? 'Khách' }}</div>
                                    ...
                                    <td class="text-muted small">
                                        {{ Str::limit($ticket->getLatestMessage()?->message ?? 'Chưa có tin nhắn', 60) }}
                                    </td>
                                    <td class="text-muted small">
            ...
            <div class="card-footer bg-white border-0 pt-0">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <small class="text-muted">
                        Hiển thị {{ $tickets->firstItem() }} - {{ $tickets->lastItem() }} / {{ $tickets->total() }} ticket
                    </small>

            ...
            <div class="lg:col-span-3 space-y-6">
                @if($trips->isEmpty())
                    <div class="glass-dark rounded-4xl p-20 flex flex-col items-center text-center space-y-6 border-none ring-1 ring-white/10">
                        ...
                        <a href="{{ route('customer.home') }}" class="liquid-gradient px-8 py-4 rounded-2xl font-black italic">VỀ TRANG CHỦ</a>
                    </div>
                @else
                    @foreach($trips as $index => $trip)
                        <div 
                            x-data="{ show: false }" 
                            x-init="setTimeout(() => show = true, {{ ($index + 1) * 100 }})"
                            x-show="show"
                            ...
                            <div class="bg-brand-dark/40 rounded-[calc(1.5rem-2px)] p-6 lg:p-8 flex flex-col lg:flex-row items-center gap-10">
                                ...
                                <div class="w-full lg:w-auto flex flex-row lg:flex-col items-center justify-between lg:justify-center gap-6 lg:pl-10 lg:border-l lg:border-white/5 shrink-0">
                                    <div class="text-right lg:text-center space-y-1">
                                        <p class="text-3xl font-black italic text-brand-accent">{{ number_format($trip->price, 0, ',', '.') }}đ</p>
                                        <p class="text-[10px] font-bold text-white/30 uppercase tracking-widest">Mỗi hành khách</p>
                                    </div>
                                    <div class="space-y-3">
                                        <a href="{{ route('customer.trips.show', $trip->id) }}" class="inline-flex items-center px-8 py-4 rounded-2xl liquid-gradient font-black italic shadow-lg shadow-brand-primary/20 hover:scale-[1.05] active:scale-[0.98] transition-all">
                                            CHỌN CHỖ
                                        </a>

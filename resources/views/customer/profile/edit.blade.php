                            ...
                            <div class="flex justify-center mb-8">
                                <div class="relative group">
                                    <div class="w-24 h-24 rounded-full bg-black/30 border-2 border-white/10 flex items-center justify-center overflow-hidden group-hover:border-brand-primary transition-colors">
                                        ...
                                        @else
                                            <span class="text-3xl font-bold text-white/30">{{ substr($user->name, 0, 1) }}</span>
                                        @endif
                                    </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white/5 border border-white/10 rounded-3xl overflow-hidden relative">
                    <div class="p-6 md:p-8 flex items-center justify-between border-b border-white/10 bg-black/20">
                        ...
                        <a href="{{ route('customer.bookings.index') }}" class="text-brand-primary font-medium hover:text-white transition-colors text-sm flex items-center gap-1">
                    ...
                    <div class="p-0">
                        @if(isset($bookings) && $bookings->isEmpty())
                            <div class="p-12 text-center text-white/50 flex flex-col items-center">
                                ...
                                <a href="{{ url('/#search') }}" class="inline-flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white font-bold py-2.5 px-6 rounded-xl transition-colors border border-white/10">
                                    Đặt vé ngay
                                </a>
                            </div>
                        @elseif(isset($bookings))
                            <div class="divide-y divide-white/5">
                                @foreach($bookings->take(5) as $booking)
                                    <a href="{{ route('customer.bookings.show', $booking->id) }}" class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 hover:bg-white/5 transition-colors group">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-white/50 flex items-center gap-3">
                                                    <span><i data-lucide="hash" class="w-3 h-3 inline"></i> {{ $booking->id }}</span>
                                                    <span><i data-lucide="calendar" class="w-3 h-3 inline"></i> {{ \Carbon\Carbon::parse($booking->trip->trip_date)->format('d/m/Y') }}</span>
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto mt-2 sm:mt-0">
                                            <div class="text-left sm:text-right">
                                                <p class="font-black text-white text-lg">{{ number_format($booking->total_amount, 0, ',', '.') }}đ</p>
                                                ...
                                                @else
                                                    <span class="text-xs font-bold text-white/50 uppercase tracking-wider">{{ $booking->status }}</span>
                                                @endif
                                            </div>

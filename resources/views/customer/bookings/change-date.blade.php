                                ...
                                <label class="block border rounded-lg p-4 cursor-pointer hover:bg-indigo-50 transition-colors">
                                    <div class="flex items-start">
                                        ...
                                        <div class="ml-3 flex-1">
                                            ...
                                            <div class="mt-3 hidden quantity-container" id="qty-{{ $trip->id }}">
                                                <div class="flex items-center gap-4">
                                                    <label class="font-medium text-sm text-gray-700">Số lượng vé muốn đổi lấy:</label>
                                                    <input type="number" name="ticket_quantity" class="qty-input w-24 px-3 py-2 border border-gray-300 rounded focus:border-amber-500 focus:outline-none" min="1" max="4" value="{{ min(4, $booking->tickets->count()) }}" disabled>
                                                    <span class="text-xs text-gray-500">(Tối đa 4 vé)</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            tripRadios.forEach(radio => {
                radio.addEventListener('change', function() {
                    ...
                    document.querySelectorAll('.quantity-container').forEach(el => {
                        ...
                        // Disable tất cả input qty để ko gửi lung tung

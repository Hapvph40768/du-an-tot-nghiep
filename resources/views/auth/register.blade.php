...
@section('auth_title', 'Bắt đầu hành trình')
@section('auth_subtitle', 'Gia nhập cộng đồng Mạnh Hùng ngay hôm nay')
    ...
    <div class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <select name="role" required
                        ...
                        <option value="customer" {{ old('role') == 'customer' ? 'selected' : '' }} class="bg-[#1a1a1a]">Khách hàng</option>
                        <option value="driver" {{ old('role') == 'driver' ? 'selected' : '' }} class="bg-[#1a1a1a]">Đối tác tài xế</option>
                    </select>
        ...
        <div class="pt-6 text-center border-t border-white/5">
            <p class="text-white/40 font-medium text-sm">
                Đã có tài khoản? 
                <a href="{{ route('login') }}" class="text-brand-accent font-black hover:underline ml-2 uppercase tracking-tighter">Đăng nhập</a>
            </p>
        </div>
    </div>
@endsection
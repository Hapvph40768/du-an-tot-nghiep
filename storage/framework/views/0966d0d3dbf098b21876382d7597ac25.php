    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="w-72 bg-white border-r border-gray-200 flex flex-col shadow-xl z-50">
            ...
            <div class="px-6 py-5 border-b border-gray-100 bg-amber-50">
                <div class="flex items-center gap-4">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(Auth::user()->avatar): ?>
                        <img src="<?php echo e(filter_var(Auth::user()->avatar, FILTER_VALIDATE_URL) ? Auth::user()->avatar : Storage::url(Auth::user()->avatar)); ?>" class="w-12 h-12 rounded-2xl object-cover" alt="Avatar">
                    <?php else: ?>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm flex items-center gap-2 font-medium text-gray-600">
                            <span class="w-2.5 h-2.5 rounded-full animate-pulse 
                                <?php echo e(in_array(Auth::user()->driver?->status, ['active', 'ready', 'available'])
                                    ? 'bg-emerald-500'
                                    : (Auth::user()->driver?->status === 'inactive'
                                        ? 'bg-amber-500'
                                        : 'bg-emerald-500')); ?>">
                            </span>
                            <?php echo e(in_array(Auth::user()->driver?->status, ['inactive']) ? 'Offline' : 'Online'); ?>
            ...
            <nav class="flex-1 px-3 py-6 overflow-y-auto">
                <ul class="space-y-1">
                    <li>
                        <a href="<?php echo e(route('driver.trips.index')); ?>" onclick="window.location.href='<?php echo e(route('driver.trips.index')); ?>'; return false;"
                            class="<?php echo e(request()->routeIs('driver.trips.index', 'driver.trips.show') ? 'bg-amber-50 text-amber-700 border-l-4 border-amber-500' : 'text-gray-600 hover:bg-gray-50'); ?> 
                    ...
                    <li>
                        <a href="<?php echo e(route('driver.trips.history')); ?>" onclick="window.location.href='<?php echo e(route('driver.trips.history')); ?>'; return false;"
                            class="<?php echo e(request()->routeIs('driver.trips.history') ? 'bg-amber-50 text-amber-700 border-l-4 border-amber-500' : 'text-gray-600 hover:bg-gray-50'); ?> 
                           flex items-center gap-3 px-5 py-4 rounded-2xl font-medium transition-all cursor-pointer">
                    ...
                    <li>
                        <a href="<?php echo e(route('driver.revenue.index')); ?>" onclick="window.location.href='<?php echo e(route('driver.revenue.index')); ?>'; return false;"
                            class="<?php echo e(request()->routeIs('driver.revenue.*') ? 'bg-amber-50 text-amber-700 border-l-4 border-amber-500' : 'text-gray-600 hover:bg-gray-50'); ?> 
                           flex items-center gap-3 px-5 py-4 rounded-2xl font-medium transition-all cursor-pointer">
...
<?php /**PATH C:\Users\admin\du-an-tot-nghiep\resources\views\layout\driver\DriverLayout.blade.php ENDPATH**/ ?>
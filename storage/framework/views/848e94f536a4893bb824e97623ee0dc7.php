    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($bookings->isEmpty()): ?>
            <div class="bg-white/5 border border-white/10 rounded-3xl p-12 text-center flex flex-col items-center justify-center">
                ...
                <a href="<?php echo e(url('/#search')); ?>" class="liquid-gradient hover:scale-105 transition-transform text-white font-bold py-3 px-8 rounded-full shadow-lg shadow-brand-primary/20 flex items-center gap-2">
                    ...
                    <div class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:bg-white/10 transition-colors group">
                        <div class="p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 relative">
                            ...
                            <div class="absolute left-0 top-0 bottom-0 w-1 
                                <?php echo e($booking->status == 'pending' ? 'bg-amber-500' : ''); ?>

                                <?php echo e($booking->status == 'paid' ? 'bg-green-500' : ''); ?>

                                <?php echo e($booking->status == 'cancelled' ? 'bg-red-500' : ''); ?>">
                            </div>

                            <!-- Left: Info -->
                            <div class="flex-1 w-full md:w-auto">
                                <div class="flex items-center gap-3 mb-4">
                                    <span class="px-3 py-1 bg-white/10 rounded-md text-xs font-mono text-white/70">#<?php echo e($booking->id); ?></span>
                                    ...
                                    <?php else: ?>
                                        <span class="px-3 py-1 bg-white/10 text-white border border-white/20 rounded-full text-xs font-bold"><?php echo e($booking->status); ?></span>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </div>

                                <div class="flex items-center gap-4 text-xl md:text-2xl font-bold text-white mb-2">
                                    <span><?php echo e($booking->trip->route->departureLocation->name ?? '...'); ?></span>
                                    <i data-lucide="arrow-right" class="text-brand-accent w-5 h-5"></i>
                                    <span><?php echo e($booking->trip->route->destinationLocation->name ?? '...'); ?></span>
                                </div>

                                <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-white/60">
                                    <div class="flex items-center gap-2">
                                        <i data-lucide="calendar" class="w-4 h-4"></i>
                                        <span><?php echo e(\Carbon\Carbon::parse($booking->trip->trip_date)->format('d/m/Y')); ?></span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <i data-lucide="clock" class="w-4 h-4"></i>
                                        <span class="text-brand-primary font-bold"><?php echo e(\Carbon\Carbon::parse($booking->trip->departure_time)->format('H:i')); ?></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="w-full md:w-auto flex flex-row md:flex-col items-center justify-between md:items-end md:justify-center gap-4 md:pl-6 md:border-l md:border-white/10">
                                <div class="text-left md:text-right">
                                    ...
                                    <p class="text-2xl font-black text-white"><?php echo e(number_format($booking->total_amount, 0, ',', '.')); ?>đ</p>
                                </div>
                                <a href="<?php echo e(route('customer.bookings.show', $booking->id)); ?>" class="inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-white text-brand-dark hover:bg-gray-200 transition-colors rounded-xl font-bold text-sm">
                                    Xem chi tiết
                                </a>
                            </div>
                        </div>
...
<?php echo $__env->make('layout.customer.CustomerLayout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\admin\du-an-tot-nghiep\resources\views/customer/bookings/index.blade.php ENDPATH**/ ?>
    ...
    <section class="py-12 lg:py-20 relative min-h-[80vh]">
        <div class="max-w-4xl mx-auto px-4 sm:px-6">
            ...
            <div class="flex items-center justify-between mb-8">
                ...
                <a href="<?php echo e(route('customer.parcels.index')); ?>"
                    class="px-4 py-2 bg-white/10 text-white border border-white/20 rounded-xl font-medium text-sm hover:bg-white/20 transition-colors flex items-center gap-2">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('success')): ?>
                <div
                    class="bg-green-500/10 border border-green-500/50 text-green-400 px-4 py-3 rounded-xl mb-6 flex items-center gap-3">
            ...
            <form action="<?php echo e(route('customer.parcels.store')); ?>" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <!-- Người gửi -->
                    <div
                        class="bg-white/5 border border-white/10 rounded-3xl p-6 md:p-8 relative overflow-hidden group hover:border-brand-primary/30 transition-colors">
                        <div
                            class="absolute top-0 right-0 w-32 h-32 bg-brand-primary/10 rounded-full blur-[40px] pointer-events-none group-hover:bg-brand-primary/20 transition-colors">
                        </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-white/70 mb-2">Họ tên người gửi <span
                                        class="text-red-400">*</span></label>
                                <div class="relative">
                                    <div
                                        class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-white/40">
                                        <i data-lucide="user" class="w-4 h-4"></i>
                                    </div>
                                    <input type="text" name="sender_name"
                                        value="<?php echo e(old('sender_name', Auth::user()->name ?? '')); ?>" required
                            ...
                            <div>
                                <label class="block text-sm font-medium text-white/70 mb-2">Số điện thoại <span
                                        class="text-red-400">*</span></label>
                                <div class="relative">
                                    <div
                                        class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-white/40">
                                    ...
                                    <input type="tel" name="sender_phone"
                                        value="<?php echo e(old('sender_phone', Auth::user()->phone ?? '')); ?>" required
                                        class="w-full px-4 py-3 pl-11 bg-black/30 border border-white/10 rounded-xl text-white focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-colors">
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Người nhận -->
                    <div
                        class="bg-white/5 border border-white/10 rounded-3xl p-6 md:p-8 relative overflow-hidden group hover:border-brand-accent/30 transition-colors">
                        <div
                            class="absolute top-0 right-0 w-32 h-32 bg-brand-accent/10 rounded-full blur-[40px] pointer-events-none group-hover:bg-brand-accent/20 transition-colors">
                        </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-white/70 mb-2">Họ tên người nhận <span
                                        class="text-red-400">*</span></label>
                                <div class="relative">
                                    <div
                                        class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-white/40">
                ...
                <div class="bg-white/5 border border-white/10 rounded-3xl p-6 md:p-8 mb-8 relative overflow-hidden">
                    <div
                        class="absolute bottom-0 left-1/2 -translate-x-1/2 w-64 h-32 bg-brand-primary/5 blur-[50px] pointer-events-none">
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            <label class="block text-sm font-medium text-white/70 mb-2">Tuyến đường gửi <span
                                    class="text-red-400">*</span></label>
                            <div class="relative">
                                <div
                                    class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-white/40">
                                ...
                                <select name="route_id" required
                                    class="w-full px-4 py-3 pl-11 bg-black/30 border border-white/10 rounded-xl text-white appearance-none focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-colors">
                                    ...
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $routes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $route): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                                        <option value="<?php echo e($route->id); ?>" <?php echo e(old('route_id') == $route->id ? 'selected' : ''); ?>
                                            ...
                                            <?php echo e($route->departureLocation->name ?? '...'); ?> &rarr;
                                ...
                                <div
                                    class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-white/40">
                                    <i data-lucide="chevron-down" class="w-4 h-4"></i>
                                </div>
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-white/70 mb-2">Cân nặng (kg) <span
                                    class="text-red-400">*</span></label>
                            <div class="relative">
                                <div
                                    class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-white/40">
                            ...
                            <p class="text-xs text-brand-accent mt-2 flex items-center gap-1">
                                <i data-lucide="info" class="w-3 h-3"></i> Cước phí ước tính: ~10.000đ/kg (Tối thiểu
                                20.000đ)
                            </p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-white/70 mb-2">Điểm nhận hàng (của nhà xe) <span class="text-red-400">*</span></label>
                            <div class="relative">
                                ...
                                <select name="pickup_point_id" id="pickup_point_id" required disabled
                                    class="w-full px-4 py-3 pl-11 bg-black/30 border border-white/10 rounded-xl text-white appearance-none focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-colors disabled:opacity-50">
                                    <option value="" class="bg-gray-900 text-white/50">-- Chọn tuyến đường trước --</option>
                                </select>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-white/70 mb-2">Điểm trả hàng (của nhà xe) <span class="text-red-400">*</span></label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-white/40">
                                    <i data-lucide="map-pin" class="w-4 h-4 text-red-400"></i>
                                </div>
                                <select name="dropoff_point_id" id="dropoff_point_id" required disabled
                                    class="w-full px-4 py-3 pl-11 bg-black/30 border border-white/10 rounded-xl text-white appearance-none focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-colors disabled:opacity-50">
                                    <option value="" class="bg-gray-900 text-white/50">-- Chọn tuyến đường trước --</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-white/40">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-white/70 mb-2">Mô tả hàng hóa <span
                                class="text-white/40 font-normal">(Loại hàng, số lượng, lưu ý...)</span></label>
                        <textarea name="description" rows="3"
                            placeholder="Ví dụ: 1 thùng quần áo, 2 tài liệu quan trọng cần nhẹ tay..."
                ...
                <div class="text-center pt-4">
                    <button type="submit"
                        class="w-full sm:w-auto liquid-gradient hover:scale-[1.02] active:scale-[0.98] transition-transform text-white font-bold py-4 px-10 rounded-xl shadow-[0_10px_40px_-10px_rgba(255,91,36,0.6)] text-lg inline-flex items-center justify-center gap-3">
                        <i data-lucide="send" class="w-5 h-5"></i> Tạo đơn ký gửi ngay
                    </button>
                    <div
                        class="mt-6 flex items-center justify-center gap-2 text-sm text-white/40 bg-black/20 py-3 px-4 rounded-xl border border-white/5 max-w-lg mx-auto">
                        <i data-lucide="alert-circle" class="w-4 h-4 text-brand-primary flex-shrink-0"></i>
                        <p>Sau khi tạo, vui lòng mang hàng hóa ra văn phòng để nhân viên kiểm tra và thanh toán báo giá
                            chính thức.</p>
                    </div>
            </form>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function loadPoints(routeId) {
            ...
            fetch(`/parcels/get-points/${routeId}`)
                .then(res => res.json())
                .then(data => {
                    // Load pickup points
                    ...
                    if (data.pickup_points.length > 0) {
                        data.pickup_points.forEach(point => {
                            ...
                            pickupSelect.innerHTML += `<option value="${point.id}" class="bg-gray-900 text-white">${point.name} ${timeText} - ${point.address}</option>`;
                        });
                    ...
                    if (data.dropoff_points.length > 0) {
                        data.dropoff_points.forEach(point => {
                            let timeText = point.time_from_departure ? ` (+${point.time_from_departure}p)` : '';
                            dropoffSelect.innerHTML += `<option value="${point.id}" class="bg-gray-900 text-white">${point.name} ${timeText} - ${point.address}</option>`;
                        });
        ...
        // Initialize if already selected (e.g. on validation error back)
        if (routeSelect.value) {
            loadPoints(routeSelect.value);
            // Optionally, we could try to re-select old values here but it requires more JS.
            // For now, the user will just have to reselect the points if validation fails.
        }
    });
</script>
<?php $__env->stopPush(); ?>

<?php $__env->startSection('page-title', 'Chuyến xe của tôi'); ?>
...
<?php $__env->startSection('content-main'); ?>
                    ...
                    <div class="flex gap-2 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-hide" id="date-filter">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php for($i = 0; $i < 7; $i++): ?>
                            <?php
                                $date = now()->addDays($i);
                            ?>
                            <button onclick="applyFilters()" data-date="<?php echo e($date->format('Y-m-d')); ?>"
                                class="date-btn flex-shrink-0 snap-center min-w-[74px] px-4 py-3.5 rounded-2xl font-medium transition-all text-center
                                        <?php echo e($i === 0 ? 'bg-blue-500 text-white shadow-md ring-2 ring-blue-200' : 'bg-white border border-gray-200 hover:border-blue-300 hover:shadow text-gray-700'); ?>">
                                <div class="text-[10px] opacity-75 tracking-widest"><?php echo e($date->format('D')); ?></div>
                                <div class="text-2xl font-semibold leading-none mt-0.5"><?php echo e($date->format('d')); ?></div>
                                <div class="text-[10px] opacity-75 mt-1"><?php echo e($date->format('m/Y')); ?></div>
                            </button>
                ...
                <div class="lg:col-span-5">
                    ...
                    <div class="flex flex-wrap gap-2" id="status-filter">
                        <button onclick="applyFilters()" data-status="active"
                            class="status-btn flex-1 sm:flex-none px-5 py-3 rounded-2xl text-sm font-medium transition-all bg-blue-500 text-white shadow">
                            Sắp chạy
                        </button>
        ...
        <?php else: ?>
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6" id="trips-container">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $trips; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trip): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                    <div class="trip-card bg-white rounded-3xl overflow-hidden border border-gray-100 hover:border-blue-200 hover:shadow-xl transition-all duration-300"
                        data-date="<?php echo e(\Carbon\Carbon::parse($trip->trip_date)->format('Y-m-d')); ?>"
                        data-status="<?php echo e($trip->status); ?>"
                        data-departure="<?php echo e(\Carbon\Carbon::parse($trip->trip_date . ' ' . $trip->departure_time)->format('Y-m-d H:i:s')); ?>">

                        <div class="h-1.5 bg-gradient-to-r from-blue-500 via-indigo-500 to-red-500"></div>

                        <div class="p-6">
                            <div class="flex justify-between items-start mb-6">
                                <div>
                                    <span
                                        class="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-2xl text-sm font-semibold">
                            ...
                            <div class="flex items-center gap-4 mb-8">
                                ...
                                <div class="text-blue-500 text-5xl -mt-2">
                            ...
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex items-baseline gap-1">
                                    ...
                                    <span class="text-gray-500"><?php echo e($trip->vehicle->total_seats ?? '?'); ?></span>
                            ...
                            <div class="flex gap-3">
                                <a href="<?php echo e(route('assistant.trips.show', $trip)); ?>"
                                    class="flex-1 text-center py-3.5 border border-blue-500 text-blue-600 font-semibold rounded-2xl hover:bg-blue-50 transition">
                                    Xem chi tiết
                                </a>
    ...
    <script>
        function applyFilters() {
            const selectedDate = document.querySelector('.date-btn.bg-blue-500')?.getAttribute('data-date') || '';
            let selectedStatus = document.querySelector('.status-btn.bg-blue-500')?.getAttribute('data-status') ||
            'active';
            ...
            tripCards.forEach(card => {
                ...
                let matchTime = true;
                // Tạm tắt filter 10h để test chức năng dễ hơn
        ...
        document.querySelectorAll('.date-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                document.querySelectorAll('.date-btn').forEach(b => {
                    b.classList.remove('bg-blue-500', 'text-white', 'shadow-md', 'ring-2',
                        'ring-blue-200');
                    b.classList.add('bg-white', 'border-gray-200', 'text-gray-700');
                });
                this.classList.add('bg-blue-500', 'text-white', 'shadow-md', 'ring-2', 'ring-blue-200');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const todayBtn = document.querySelector(`.date-btn[data-date="<?php echo e(now()->format('Y-m-d')); ?>"]`);
            if (todayBtn) {
                todayBtn.classList.add('bg-blue-500', 'text-white', 'shadow-md', 'ring-2', 'ring-blue-200');
            }
...
<?php echo $__env->make('layout.assistant.AssistantLayout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\du-an-tot-nghiep\resources\views/assistant/trips/index.blade.php ENDPATH**/ ?>